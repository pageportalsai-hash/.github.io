// VIALBREAK single 0-100 containment progression with 770 unique specimens.
(function(){
  const MAX_CONTAINMENT=100;
  const TOTAL_SPECIMENS=770;
  ...
  // Preserve the original 670 ordering exactly, then append 100 new endgame specimens.
  const rarityPlan=[
    ['Stable',180],['Oddity',145],['Mutant',120],['Hazard',95],['Black Label',65],['Impossible',45],['Apex',20],
    ['Relic',60],['Singularity',40]
  ];
  const prefixes=[
    'Glass','Velvet','Radio','Copper','Midnight','Static','Paper','Neon','Hollow','Lucky','Crimson','Silver','Ghost','Pocket','Signal','Orbit','Dream','Mirror','Clock','Cinder','Frost','Lunar','Echo','Zero','Violet','Amber','Chrome','Moss','Comet','Marble','Electric','Rust','Bubble','Quartz','Velcro','Solar','Dust','Candy','Ink','Pixel','Ion','Prism','Halo','Cipher','Titan','Nova','Aether','Obsidian','Aurora','Vector'
  ];
  const suffix={
  ...
  const rarityBase={Stable:20,Oddity:65,Mutant:175,Hazard:650,'Black Label':2600,Impossible:18000,Apex:52000,Relic:110000,Singularity:240000};
  const palettes=['#76e6c2','#7fb5ff','#f0c469','#d98cff','#ff8077','#8ee76f','#e8eef0','#68e3ff','#ff9bcb','#a995ff','#f8a35f','#69d6ff','#d6ff80','#ff758f','#54f4ff','#fff0a4','#b7fff7','#ffd86e'];
  const darks=['#17483b','#1d3851','#4a3518','#3c214d','#4d2024','#29441e','#394344','#16414a','#4b2239','#30254c','#4a2b18','#16384c','#31401c','#4c1a26','#113a45','#4a3a10','#1a3f3b','#51450f'];
  ...
  const rackDefs=[
    {id:'starter',name:'BASIC SAMPLE RACK',sub:'LOW COST // STANDARD',price:120,god:.002,odds:'God Rack 1:500',a:'#2a6b50',b:'#102b22',glow:'#75ffad',weights:{Stable:60,Oddity:25,Mutant:10,Hazard:4,'Black Label':.8,Impossible:.18,Apex:.02,Relic:.003,Singularity:.0005}},
    {id:'quarantine',name:'ENRICHED RACK',sub:'AFFORDABLE // BETTER',price:300,god:.003,odds:'God Rack 1:333',a:'#2c6573',b:'#142b38',glow:'#70ecff',weights:{Stable:46,Oddity:29,Mutant:16,Hazard:6.5,'Black Label':2,Impossible:.42,Apex:.08,Relic:.01,Singularity:.002}},
    {id:'research',name:'RESEARCH CASE',sub:'MID RANGE // STRONG',price:750,god:.0045,odds:'God Rack 1:222',a:'#66562d',b:'#332a12',glow:'#ffe27a',weights:{Stable:34,Oddity:29,Mutant:20,Hazard:10,'Black Label':5.5,Impossible:1.3,Apex:.2,Relic:.04,Singularity:.008}},
    {id:'blacksite',name:'BLACKSITE CRATE',sub:'EXPENSIVE // RARE HUNT',price:5500,god:.007,odds:'God Rack 1:143',a:'#662f3d',b:'#31151d',glow:'#ff718a',weights:{Stable:22,Oddity:27,Mutant:24,Hazard:15,'Black Label':8.5,Impossible:2.8,Apex:.7,Relic:.12,Singularity:.03}},
    {id:'omega',name:'OMEGA VAULT',sub:'PREMIUM // VERY GOOD',price:13500,god:.01,odds:'God Rack 1:100',a:'#4b356d',b:'#24172f',glow:'#cf86ff',weights:{Stable:13,Oddity:22,Mutant:26,Hazard:20,'Black Label':12.5,Impossible:5.2,Apex:1.3,Relic:.35,Singularity:.1}},
    {id:'apex',name:'APEX ARCHIVE',sub:'CHASE RACK // BEST ODDS',price:30000,god:.015,odds:'God Rack 1:67',a:'#74622b',b:'#2c2410',glow:'#ffe16c',weights:{Stable:7,Oddity:16,Mutant:23,Hazard:23,'Black Label':18,Impossible:10,Apex:3,Relic:.8,Singularity:.25}}
  ];
  racks.splice(0,racks.length,...rackDefs.map(r=>({...r,level:1})));

  boostedWeights=function(base){const w={...base};w.Stable*=.18;w.Oddity*=.55;w.Mutant*=1.25;w.Hazard*=1.8;w['Black Label']*=2.5;w.Impossible*=3.2;w.Apex*=3.8;w.Relic*=4.5;w.Singularity*=5.2;return w};
  godWeights=function(){return{Stable:0,Oddity:0,Mutant:14,Hazard:31,'Black Label':27,Impossible:16,Apex:8,Relic:3.2,Singularity:.8}};
  ...
  function syncContainmentUI(){
    ...
    const archiveP=document.querySelector('#screen-collection .section-head p');if(archiveP)archiveP.textContent='Containment is your percentage completion of the full 770-vial collection. No resets. No floors. No levels.';
    ...
    const note=document.querySelector('#screen-shop .section-head .note');if(note)note.textContent='6 rack classes • 7 sealed vials • Final Chamber boost • 9 rarity classes';
    const chase=[...specimens].filter(s=>s.rarity==='Singularity').sort((a,b)=>b.base-a.base)[0];
    if(chase){const h=document.querySelector('.chase .panel-head h3');if(h)h.textContent=chase.name;const chip=document.querySelector('.chase .chip');if(chip){chip.textContent='SINGULARITY';chip.style.color=colors.Singularity}const body=document.querySelector('.chasebody');if(body)body.innerHTML=`<div class="hero-vial impossible-vial singularity-vial" style="--liqA:${chase.liqA};--liqB:${chase.liqB}"><div class="cap"></div><div class="glass"><div class="liquid"></div><div class="organism">${chase.glyph}</div></div><small>SINGULARITY SAMPLE</small></div><div><p>${chase.text}</p><div class="kv"><span>RAW INDEX</span><b>₡ ${fmt(chase.base)}</b></div><div class="kv"><span>CGC 10 EST.</span><b class="gold">₡ ${fmt(chase.base*5.2)}</b></div></div>`;}
