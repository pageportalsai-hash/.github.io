...
(function(){
  ...
  tools.splice(0,tools.length,...toolDefs.map((x,i)=>({id:i,name:x[0],emoji:x[1],unlock:x[2],price:x[3],time:x[4],desc:i>=AUTO_INDEX?'Autonomous vial cracking and sorting.':'Manual precision cracking tool.'})));
  ...
  renderTools=function(){
    ...
    $('#currentToolDisplay').innerHTML=`<div class="emoji">${current.emoji}</div><b>${current.name}</b><small>Containment ${c}/100 • ${(current.time/1000).toFixed(2)} sec/vial</small><div class="tool-status ${autoReady?'auto-ready':''}">${autoReady?'AUTO CRACK INSTALLED':'AUTO CRACK UNLOCKS AT CONTAINMENT 70'}</div>`;
    $('#toolGrid').innerHTML=tools.map((x,i)=>{const locked=c<x.unlock,currentTool=i===state.tool,passed=i<state.tool,next=i===state.tool+1,canBuy=next&&!locked&&state.credits>=x.price;const label=currentTool?'EQUIPPED':passed?'OWNED':locked?`CONTAINMENT ${x.unlock}`:next?`BUY • ₡ ${fmt(x.price)}`:'BUY PREVIOUS TOOL';return `<div class="tool-card ${currentTool?'current':''} ${locked?'tool-locked':''} ${i>=AUTO_INDEX?'auto-tier':''}"><div class="tool-level">CONTAINMENT ${x.unlock}</div><div class="emoji">${x.emoji}</div><h3>${x.name}</h3><p>${x.desc}</p><div class="speed">CRACK TIME ${(x.time/1000).toFixed(2)}s</div>${i===AUTO_INDEX?'<div class="billion-badge">AUTO CRACK • ₡750,000</div>':''}<button class="upgrade-btn" data-tool="${i}" ${!canBuy?'disabled':''}>${label}</button></div>`}).join('');
